...
from ray_diffusion.model.diffuser_6dof_layer import RayDiffuser
...
# from ray_diffusion.lietorch import SE3
...
class Trainer(object):
            ...
            for batch in self.train_dataloader:
                ...
                with torch.cuda.amp.autocast(enabled=self.mixed_precision):
                    ...
                    for i in range(R.shape[0]):
                        target_transform_matrix = self.generate_transformation_matrix_batch(R[i], T[i])
                        # print("target_transform_matrix", target_transform_matrix.shape) # torch.Size([2, 4, 4])
                        target_lie_alg = SE3.from_matrix(target_transform_matrix, normalize=True).log().to(self.device)
                        # print("target_lie_alg", target_lie_alg.shape) # ([2, 6])
                        lie_alg_gt.append(target_lie_alg)
                    lie_alg_gt = torch.stack(lie_alg_gt, dim=0)
                    # print("lie_alg_gt", lie_alg_gt.shape) # ([8, 2, 6])
                    ...
                    # print("rays_tensor.shape", rays_tensor.shape) # torch.Size([8, 2, 6)
                    ...
                    loss = torch.mean((eps_pred.squeeze(-1).squeeze(-1) - target) ** 2)
    ...
    def lie_loss(self, eps_pred, target, cameras):
        ...
        for cam_idx in range(len(cameras)):
            ...
            # transform_matrix = self.generate_transformation_matrix_batch(pred_cam.R.to('cpu'), pred_cam.T.to('cpu'))
            # transform_matrix = self.generate_transformation_matrix_batch(pred_cam.R.cpu().detach().numpy(), pred_cam.T.cpu().detach().numpy())
            transform_matrix = self.generate_transformation_matrix_batch(pred_cam.R, pred_cam.T) # liegroups.torch.SE3 does not support gpu tensor
            # Initialize a list to store the Lie algebra representations
            # Number of matrices in the batch
            # N = pred_cam.R.shape[0]
            
            # # Initialize a batch of 4x4 identity matrices on the same device as R
            # transform_matrix = torch.eye(4, device=pred_cam.R.device).unsqueeze(0).repeat(N, 1, 1)
            
            # # Set the top-left 3x3 part to the rotation matrices R
            # transform_matrix[:, :3, :3] = pred_cam.R
            
            # # Set the top-right 3x1 part to the translation vectors t
            # transform_matrix[:, :3, 3] = pred_cam.T
            lie_algebras = []

            # print(pred_cam.R[0].shape)
            # print(pred_cam.T[0].shape)
            
            # Loop through each transformation matrix and compute its Lie algebra representation
            for i in range(transform_matrix.shape[0]):
                # lie_alg = self.generate_lie_algebra(pred_cam.R[i], pred_cam.T[i])
                lie_alg = self.generate_lie_algebra(transform_matrix[i])
                # lie_alg = SE3.from_matrix(transform_matrix[i],normalize=True).log()
                # print("lie_alg", lie_alg)
                lie_algebras.append(torch.tensor(lie_alg.requires_grad_(True)))

            # Convert the list of Lie algebra representations to a tensor
            # print("lie_algebras", lie_algebras)
            lie_algebras_tensor = torch.stack(lie_algebras)
            # all_lie_algebras_pred.append(lie_algebras)
        ...
        all_lie_algebras_pred_tensor = torch.stack(all_lie_algebras_pred, dim=0).requires_grad_(True)
        # print("all_lie_algebras_pred_tensor", all_lie_algebras_pred)
        # print("all_lie_algebras_pred_tensor", all_lie_algebras_pred_tensor)  # (B, 2, 6)
        all_lie_algebras_pred_tensor = all_lie_algebras_pred_tensor.to(pred_cam.R.device) # to gpu
        # print("all_lie_algebras_tensor.shape", all_lie_algebras_pred_tensor.shape)  # (B, 2, 6)
        ...
        for cam_idx in range(len(cameras)):
            ...
            transform_matrix = self.generate_transformation_matrix_batch(target_cam.R, target_cam.T)
            # Number of matrices in the batch
            # N = pred_cam.R.shape[0]
            
            # # Initialize a batch of 4x4 identity matrices on the same device as R
            # transform_matrix = torch.eye(4, device=target_cam.R.device).unsqueeze(0).repeat(N, 1, 1)
            
            # # Set the top-left 3x3 part to the rotation matrices R
            # transform_matrix[:, :3, :3] = target_cam.R
            
            # # Set the top-right 3x1 part to the translation vectors t
            # transform_matrix[:, :3, 3] = target_cam.T
            # Initialize a list to store the Lie algebra representations
            lie_algebras = []
            ...
            for i in range(transform_matrix.shape[0]):
                lie_alg = self.generate_lie_algebra(transform_matrix[i])
                # lie_alg = SE3.from_matrix(transform_matrix[i],normalize=True).log()
                lie_algebras.append(torch.tensor(lie_alg))
        ...
        # Stack all Lie algebra representations to form a tensor with shape [B, 2, 6]
        all_lie_algebras_target_tensor = torch.stack(all_lie_algebras_target, dim=0)
        ...
        angle_loss = torch.nn.functional.mse_loss(all_lie_algebras_pred_tensor[:, :, :3], all_lie_algebras_target_tensor[:, :, :3]).requires_grad_(True)

        # Compute the translation loss (last three elements [3:6])
        translation_loss = torch.nn.functional.mse_loss(all_lie_algebras_pred_tensor[:, :, 3:], all_lie_algebras_target_tensor[:, :, 3:]).requires_grad_(True)

        # Compute the total loss with different weights
        # lie_alg_loss = angle_loss + 100 * translation_loss
        print("angle_loss:", angle_loss)
        print("translation_loss:", translation_loss)
        lie_alg_loss = angle_loss + translation_loss
        print("total_lie_alg_loss:", lie_alg_loss)
        
        print("angle_loss.requires_grad:", angle_loss.requires_grad)  # Should be True
        print("translation_loss.requires_grad", translation_loss.requires_grad)  # Should be True
        print(all_lie_algebras_pred_tensor.requires_grad)  # Should be True
        print(all_lie_algebras_target_tensor.requires_grad)  # Can be False (targets don't need grads)
        print(all_lie_algebras_pred_tensor.grad_fn) # None
        print(lie_alg_loss.grad_fn)  # <AddBackward0 object at 0x7f8cd03583d0>
    ...
    def generate_lie_algebra(self, matrix):
        ...
        # print("matrix", matrix)
        # lie_algebra = SE3(rot=R, trans=t).log()
