...
from ray_diffusion.utils.rays import cameras_to_rays_for_rec
...
class Trainer(object):
    ...
    def load_model(self, path, load_metadata=True):
        ...
        # Adjust the size of the parameter 'noise_scheduler.alphas_cumprod'
        if 'noise_scheduler.alphas_cumprod' in checkpoint_state_dict:
            mismatched_param = checkpoint_state_dict['noise_scheduler.alphas_cumprod']
            adjusted_param = torch.nn.functional.interpolate(mismatched_param.unsqueeze(0).unsqueeze(0), size=(1000,), mode='nearest').squeeze(0).squeeze(0)
            checkpoint_state_dict['noise_scheduler.alphas_cumprod'] = adjusted_param
        
        # Adjust the size of the parameter 'noise_scheduler.betas'
        if 'noise_scheduler.betas' in checkpoint_state_dict:
            mismatched_param = checkpoint_state_dict['noise_scheduler.betas']
            adjusted_param = torch.nn.functional.interpolate(mismatched_param.unsqueeze(0).unsqueeze(0), size=(1000,), mode='nearest').squeeze(0).squeeze(0)
            checkpoint_state_dict['noise_scheduler.betas'] = adjusted_param
        
        # Adjust the size of the parameter 'noise_scheduler.alphas'
        if 'noise_scheduler.alphas' in checkpoint_state_dict:
            mismatched_param = checkpoint_state_dict['noise_scheduler.alphas']
            adjusted_param = torch.nn.functional.interpolate(mismatched_param.unsqueeze(0).unsqueeze(0), size=(1000,), mode='nearest').squeeze(0).squeeze(0)
            checkpoint_state_dict['noise_scheduler.alphas'] = adjusted_param
