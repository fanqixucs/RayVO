...
print("using identity and forward odometry")
...
class Co3dDataset(Dataset):
    ...
    def get_data(self, index=None, sequence_name=None, ids=(0, 1), no_images=False):
        ...
        odometry_R = torch.stack([torch.eye(3), torch.tensor(odometry_inverse_transformation[:3, :3])], dim=0)
        odometry_T = torch.stack([torch.zeros(3), torch.tensor(odometry_inverse_transformation[:3, 3])], dim=0)
