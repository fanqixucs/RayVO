...
class Rays(object):
    def __init__(
        ...
        if ndc_coordinates is not None:
            ...
            # print("xy_grid from compute ndc using crop center",xy_grid)
        else:
            self.ndc_coordinates = None
            # for test, why ndc_coordinates is None
            # xy_grid_raw = compute_ndc_coordinates(
            #     crop_parameters,
            #     num_patches_x=num_patches_x,
            #     num_patches_y=num_patches_y,
            # )
            # # print("xy_grid_raw",xy_grid_raw)
            # xy_grid = compute_ndc_coordinates(
            #     crop_parameters,
            #     num_patches_x=num_patches_x,
            #     num_patches_y=num_patches_y,
            # )[..., :2]
            # # print("xy_grid = xy_grid_raw[..., :2]",xy_grid)
            # xy_grid = xy_grid.reshape(*xy_grid.shape[:-3], -1, 2)
            # # print("xy_grid = xy_grid.reshape(*xy_grid.shape[:-3], -1, 2)",xy_grid)
            # print("xy_grid for append",xy_grid)
            # self.ndc_coordinates = xy_grid
            # self.ndc_coordinates = None
    ...
    def to_spatial(self, include_ndc_coordinates=False, num_patches_x=None, num_patches_y=None):
        """
    ...
    def rescale_moments_only_z(self, scale):
        """
        Rescale the moment component of the rays by a scalar. Might be desirable since
        moments may come from a very narrow distribution.
    ...
    def to_point_direction_moments_rescale(self, normalize_moment=True):
        """
        Convert to point direction representation <O, D>.
        ...
        if self._is_plucker:
            direction = torch.nn.functional.normalize(self.rays[..., :3], dim=-1)
            moment = self.rays[..., 3:]
            # Divide the last element of the moment by 100
            moment[..., -1] /= 100
            if normalize_moment:
                c = torch.linalg.norm(direction, dim=-1, keepdim=True)
                moment = moment / c
    ...
    def to_point_direction_whole_moments_rescale(self, normalize_moment=True):
        """
        Convert to point direction representation <O, D>.
    ...
    def to_plucker(self):
        ...
        if self.is_plucker:
            ...
            # print("ray_directions.norm(dim=-1, keepdim=True)",ray_directions.norm(dim=-1, keepdim=True))
            # print("ray_origins for cross production",ray_origins)
            ray_directions = ray_directions / ray_directions.norm(dim=-1, keepdim=True)
            plucker_normal = torch.cross(ray_origins, ray_directions, dim=-1)
            # print("plucker_normal moment",plucker_normal)
    ...
    def get_origins_moments_rescale(self):
        if self.is_plucker:
            origins = self.to_point_direction_moments_rescale().get_origins_moments_rescale()
        else:
            origins = self.rays[..., :3]
        return origins
    
    def get_origins_whole_moments_rescale(self):
        if self.is_plucker:
            origins = self.to_point_direction_whole_moments_rescale().get_origins_whole_moments_rescale()
        else:
            origins = self.rays[..., :3]
        return origins
...
def cameras_to_rays_for_rec(
    ...
    for camera, crop_param in zip(cameras, crop_parameters_list):
        xyd_grid = compute_ndc_coordinates_for_debug_rec(
        ...
        '''
        # print("xyd_grid original",xyd_grid_orginal)
        # print("xyd_grid",xyd_grid)
        # print("xyd_grid.reshape(-1, 3)",xyd_grid.reshape(-1, 3))
        unprojected.append(
    ...
    # print("origins in world coordinate\n",origins)
    directions = unprojected - origins
    # print("directions in world coordinate\n",directions)  
    # print("directions",directions * 0.26) # check the plot_scene is ploting the right ray
    rays = Rays(
        origins=origins,
...
def cameras_to_rays_for_rec_rescale_moments(
    cameras,
    crop_parameters,
    use_half_pix=True,
    use_plucker=True,
    ...
    if use_plucker:
        return rays.to_plucker().rescale_moments(moments_rescale)
    return rays

def cameras_to_rays_for_rec_rescale_moments_only_z(
    cameras,
    crop_parameters,
    use_half_pix=True,
    use_plucker=True,
    ...
    if use_plucker:
        return rays.to_plucker().rescale_moments_only_z(moments_rescale_only_z)
    return rays



def cameras_to_rays_for_debug_rec(
    cameras,
    crop_parameters,
    use_half_pix=True,
    use_plucker=True,
    ...
    if use_plucker:
        return rays.to_plucker(),xyd_grid.reshape(-1, 3), origins, unprojected, directions
    return rays


def cameras_to_rays_for_debug(
    cameras,
    crop_parameters,
    use_half_pix=True,
    use_plucker=True,
    ...
    for camera, crop_param in zip(cameras, crop_parameters_list):
        xyd_grid, xyd_grid_orginal, xyd_original_bounder = compute_ndc_coordinates_for_debug(
            crop_parameters=crop_param,
            use_half_pix=use_half_pix,
            num_patches_x=num_patches_x,
            num_patches_y=num_patches_y,
    ...
    if use_plucker:
        return rays.to_plucker(),xyd_grid.reshape(-1, 3), xyd_grid_orginal.reshape(-1, 3), origins, unprojected, directions
    return rays, xyd_grid, xyd_grid_orginal, xyd_original_bounder, unprojected
...
def rays_to_cameras_for_rec(
    ...
    # print("I_camera intrinsic\n",I_camera.get_projection_transform().get_matrix())
    I_patch_rays = cameras_to_rays_for_debug_rec(
        cameras=I_camera,
        num_patches_x=num_patches_x,
        num_patches_y=num_patches_y,
        use_half_pix=use_half_pix,
    ...
    for i in range(len(I_camera)):
        R[i] = compute_optimal_rotation_alignment(
            I_patch_rays[i].to(torch.float64),
            directions[i].to(torch.float64),
        )

    # Construct and return rotated camera
    cam = I_camera.clone()
    cam.R = R
    # cam.T = -torch.matmul(R, camera_centers.unsqueeze(2)).squeeze(2)
    cam.T = -torch.matmul(R.transpose(1, 2).cuda(), camera_centers.unsqueeze(2).double().cuda()).squeeze(2)
    return cam

def rays_to_cameras_for_rec_moments_rescale(
    rays,
    crop_parameters,
    num_patches_x=16,
    num_patches_y=16,
    ...
    """
    device = rays.device
    origins = rays.get_origins_moments_rescale()
...
def rays_to_cameras_for_rec_whole_moments_rescale(
    rays,
    crop_parameters,
    num_patches_x=16,
    num_patches_y=16,
    ...
    """
    device = rays.device
    origins = rays.get_origins_whole_moments_rescale()
    # print("origins",origins)
    directions = rays.get_directions()
    # print("directions",directions)
    camera_centers, _ = intersect_skew_lines_high_dim(origins, directions)
...
def compute_ndc_coordinates_for_debug(
    crop_parameters=None,
    use_half_pix=True,
    num_patches_x=16,
    num_patches_y=16,
    ...
    if crop_parameters is None:
        ...
        if len(crop_parameters.shape) > 1:
            return torch.stack(
                [
                    compute_ndc_coordinates_for_debug(
                        crop_parameters=crop_param,
                        use_half_pix=use_half_pix,
                        num_patches_x=num_patches_x,
                        num_patches_y=num_patches_y,
        ...
        # print("cc_x, cc_y, width",cc_x, cc_y, width)

    dx = 1 / num_patches_x # half of the width of each patch
    # print("dx",dx)
    dy = 1 / num_patches_y
    # print("dy",dy)
    if use_half_pix:
        ...
        min_x = 1 - dx
        max_x = -min_x
        bounder = torch.tensor([min_y, max_y, min_x, max_x], device=device)
        # print("min_y, max_y, min_x, max_x using half_pix",min_y, max_y, min_x, max_x)
    else:
        min_y = min_x = 1
        max_y = -1 + 2 * dy
        max_x = -1 + 2 * dx
        # print("min_y, max_y, min_x, max_x wo using half_pix",min_y, max_y, min_x, max_x)
    ...
    xyd_original_grid = torch.stack([x, y, torch.ones_like(x)], dim=-1)
    # print("y, x",y, x)
    ...
    # print("xyd_grid",xyd_grid)
    return xyd_grid, xyd_original_grid, bounder


def compute_ndc_coordinates_for_debug_rec(
    crop_parameters=None,
    use_half_pix=True,
    num_patches_x=16,
    num_patches_y=16,
    ...
    dx = width / num_patches_x # half of the width of each patch
    # print("dx",dx)
    dy = 1 / num_patches_y
    # print("dy",dy)
    if use_half_pix:
    ...
    xyd_original_grid = torch.stack([x, y, torch.ones_like(x)], dim=-1)
    # print("y, x",y, x)
    # x_prime = x * width / scale - cc_x
    # y_prime = y * width / scale - cc_y
    # # print("x_prime, y_prime",x_prime, y_prime)
    # xyd_grid = torch.stack([x_prime, y_prime, torch.ones_like(x)], dim=-1)
    # print("xyd_grid",xyd_grid)
    # return xyd_grid, xyd_original_grid, bounder
    return xyd_original_grid
