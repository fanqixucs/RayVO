...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_pos_enc = FeaturePositionalEncoding(
            max_num_images, hidden_size, width*height, P=self.P
        )
    ...
    def unpatchify(self, x):
        ...
        # print(h, w, p, c, x.shape[1]) # 24 24 1 6 585 24*24 = 576
        h = self.height
        w = self.width
    ...
    def forward(self, x, t):
        ...
        """
        B, N, c, h, w = x.shape
        # print("x.shape", x.shape) # torch.Size([8, 2, 392, 13, 45])
        P = self.P

        x = x.reshape((B * N, c, h, w))  # (B * N, C, H, W)
        # x = self.x_embedder(x.to(torch.float16))  # (B * N, C, H / P, W / P)
        x = self.x_embedder(x)
        ...
        x = x.reshape((B * N, h * w // P**2, self.out_channels * P**2))
        # print("x.shape", x.shape) # torch.Size([16, 585, 6]) # 585 = 13 * 45
